...
export class BudgetInformationComponent implements OnInit, OnDestroy {
  budgetInformation: any;
  budgetType: string = "";
  currentLang: string = "en-US";
  currentUser: any;
  ...
  private getBudgetInformation(): void {
    if (this.currentUser && this.currentUser.isManager) {
      this.departmentId = this.currentUser.departmentId;
    }
    this.budgetInformationService.getBudgetInformation(this.departmentId).subscribe(
      (data) => {
  ...
  getBudgetType(): string {
    return this.translateService.currentLang === 'en-US' ?
      `Budget Type - ${this.getBudgetTypeName()}` :
      `סוג תקציב - ${this.getBudgetTypeName()}`;
  }
